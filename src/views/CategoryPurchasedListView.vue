    ...
    <div v-if="category_results !== false">
      <b-card-group class="d-flex justify-content-center">
        <b-row sm="12" lg="3" align-h="center">
          <item-component-purchased-list v-for="item in category_results" :key="item.id" :id="item.id" :text="item.title" :imageurl="item.imageurl" :category="item.category" :date="item.date" :price="item.price" />
        </b-row>
      </b-card-group>
    </div>
    <div v-else>
...
export default {
  ...
  watch: {
    $route(to, from) {
        ...
        this.purchased_results = false;
        this.category_results = false;
        this.$bind('purchased_results', db.collection('purchasedlist').where('user', '==', auth.currentUser.uid))
        this.$bind('category_results', db.collection('purchasedlist').where('user', '==', auth.currentUser.uid).where('category', '==', this.$route.params.category))
    }
  },
  firestore: function() {
    return {
      purchased_results: db.collection('purchasedlist').where('user', '==', auth.currentUser.uid),
      category_results: db.collection('purchasedlist').where('user', '==', auth.currentUser.uid).where('category', '==', this.$route.params.category)
    }
  }
}
