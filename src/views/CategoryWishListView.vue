    ...
    <div v-if="category_results !== false">
      <b-card-group class="d-flex justify-content-center">
        <b-row sm="12" lg="3" align-h="center">
          <item-component-wish-list v-for="item in category_results" :key="item.id" :id="item.id" :text="item.title" :link="item.link" :imageurl="item.imageurl" :tags="item.tags" />
        </b-row>
      </b-card-group>
    </div>
    <div v-else>
...
export default {
  ...
  watch: {
    $route(to, from) {
        ...
        this.wishlist_results = false;
        this.category_results = false;
        this.$bind('wishlist_results', db.collection('wishlist').where('user', '==', auth.currentUser.uid))
        this.$bind('category_results', db.collection('wishlist').where('user', '==', auth.currentUser.uid).where('tags', 'array-contains', this.$route.params.category))
    }
  },
  ...
  firestore: function() {
    return {
      ...
      category_results: db.collection('wishlist').where('user', '==', auth.currentUser.uid).where('tags', 'array-contains', this.$route.params.category)
